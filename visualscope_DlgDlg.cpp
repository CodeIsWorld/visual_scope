...
/////////////////////////////////////////////////////////////////////////
	...
	receive_buffer=new unsigned char[MaxTotalBufNum];
	receive_time=new long int[MaxTotalBufNum];
	...
	tRect1.bottom=tRect.bottom-30;
	tRect_disp_channel.left=tRect.left;	
	tRect_disp_channel.right=tRect.right;
	tRect_disp_channel.top=tRect.bottom-30;
	tRect_disp_channel.bottom=tRect.bottom;
	...
	scope_data_time=new long int[max_point_num];
	memset(scope_data_time,0,sizeof(long int)*max_point_num);
	point_num=0;
	point_num_temp=0;
	point_num_old=0;
	bad_num=0;
...
{	
	...
	if (!time_start_flag)//串口第一个数据到来开始计时
	{
		current_time = clock();
		//GetLocalTime( &sys );
		time_start_flag=1;
	...
	long int time_temp=clock();
	receive_time[ctTotalBufNum]=time_temp-current_time;
	//strTemp.Format("time_temp=%ld,current_time=%ld,receive_time[0]=%ld,receive_time[1]=%ld,ctTotalBufNum=%d",time_temp,current_time,receive_time[0],receive_time[1],ctTotalBufNum);
	//if (ctTotalBufNum==1)
...
void CVisualscope_DlgDlg::ReceiveDataDecode(unsigned char *buffer_in,	signed short int(* buff_out)[4])
{
	...
	while (temp_index+10<=ctTotalBufNum)
	{
		...
		if (CRC16==CRC16_receiv)//校验合格，数据有效
		{
			...
			scope_data_time[point_num_temp]=receive_time[temp_index-10];//提取10个字节的第一个字节的时间作为当前通道数据的时间
	...
	memmove(receive_time,&receive_time[temp_index],move_num);
...
void CVisualscope_DlgDlg::Display(CDC *pDC,signed short int(* buff_out)[4])
{
	...
	num_to_change=point_num-disp_num;
	if(num_to_change<=0)
			num_to_change=0;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
 	//x轴坐标值   //rxh1.6
	int X_zuobiao_first=0;
	int x_total=point_num/XDispScale; //根据放大倍数，确定一共需要几个坐标点来显示
	
	int num_x=tRect1.right/50;//X轴显示的坐标个数

	for (i=1;i<num_x+1;i++)
	{
		long int time_temp=scope_data_time[disp_point_start+i*XDispScale-1];

		char hour=0,minute=0,second=0;
// 		if (time_temp/3600000>0)//大于一小时
// 		{
		hour=time_temp/3600000;
		minute=time_temp/60000%60;
		second=time_temp/1000%60;
	//	}
// 		if (time_temp%60/60000>0)//分钟数大于0
// 		{
// 		}
		string_temp.Format( "%ld",time_temp%1000);
		pDC->TextOut(X_basic-7+i*50,Y_basic+4,string_temp);
		//点的序号
		//string_temp.Format( "%ld",disp_point_start+i*XDispScale);	
		string_temp.Format( "%02d:%02d:%02d",hour,minute,second);
		pDC->TextOut(X_basic-20+i*50,Y_basic-15,string_temp);
 	}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 	//y轴坐标值
	...
	//底部4通道矩形
	CBrush tBrush1(RGB(100,238,128));
	pDC->SelectObject(&tBrush1);
	pDC->Rectangle(tRect_disp_channel);
	...
	if(disp_scope_flag)
	{
		...
		{
			if(disp_channl[j]&&disp_biaochi==1)//如果显示
			{
				int index_offset = (mouse_point.x-X_basic-(25/XDispScale))*XDispScale/50; //index_offset = (mouse_point.x-X_basic-(50/XDispScale)+(25/XDispScale))/(50/XDispScale); 
				int x_temp1=X_basic+(index_offset+1)*50/XDispScale, //因为第一个数 向后50像素
				y_real_value=buff_out[disp_point_start+index_offset][j],
				y_temp1=Y_basic-y_real_value*40/YDispScale;

				pDC->SelectObject(&n_font);
				pDC->SetTextColor(color[j]);//设置对话框字体的颜色
				pDC->SetBkColor(RGB(28,28,28));//设置字体背景颜色
				pDC->SetBkMode(2);//设置控件背景透明
				string_temp.Format("%d",y_real_value);	//鼠标旁边打印通道值
				pDC->TextOut(x_temp1+5,y_temp1-5,string_temp);
				//在底部窗口打出通道值
				pDC->SelectObject(&x_font);
				pDC->SetBkMode(0);//设置控件背景透明
				string_temp.Format("通道%d: %d",j+1,y_real_value);
				pDC->TextOut(tRect_disp_channel.left+tRect_disp_channel.Width()/6*(j+1),tRect_disp_channel.top+6,string_temp);
...
//	string_temp.Format( "%4d/%02d/%02d %02d:%02d:%02d.%03d 星期%1d\n", sys.wYear,sys.wMonth,  sys.wDay, sys.wHour, sys.wMinute, sys.wSecond,sys.wMilliseconds,sys.wDayOfWeek);
	//string_temp.Format("current_time=%ld,scope_data_time[0]=%ld,scope_data_time[1]=%ld",current_time,scope_data_time[0],scope_data_time[1]);
...
void CVisualscope_DlgDlg::OnTimer(UINT nIDEvent) 
{
	switch(nIDEvent)
	{
		case 1:	ReceiveDataDecode(receive_buffer,scope_data);
			...
			InvalidateRect(&tRect_disp_channel,FALSE);
...
void CVisualscope_DlgDlg::OnDestroy() 
{
	...
	if(receive_time != NULL)//rxh2.8删除自开辟的内存
    {
        delete [] receive_time;
        receive_time = NULL;//赋值空指针，防止乱删空间
    }
	if(scope_data!=NULL)
	{
		delete[]scope_data;
...
// 		dc.LineTo(point);//绘制线段
		...
		int tmp_mouse_move = mouse_move_x - mouse_move_x_old;
		mouse_move_x_old = mouse_move_x;
		scoll_position =(scoll_position*1.0/max_scoll_position - tmp_mouse_move*1.0/50*XDispScale/num_to_change)*max_scoll_position;
...
void CVisualscope_DlgDlg::SaveSoftwareConfig()
{
	...
	int data[10]={0};
	data[0]=Com_Num;//端口号
	data[1]=m_Combo_bandRate.GetCurSel();//波特率
	if(m_check_positive_only.GetCheck())//显示正数还是负数
 	...
 	data[3]=XDispScale;
 	data[4]=YDispScale;
	data[5]=m_check_y_auto.GetCheck()==true? 1:0;
...
void CVisualscope_DlgDlg::OpenSoftwareConfig()
{
		...
		{
			y_auto=1;
			m_check_y_auto.SetCheck(1);
		}
		else
		{
			y_auto=0;
			m_check_y_auto.SetCheck(0);
		}
