		...
		*/
		if (SetCommMask(m_hComm, dwCommEvents))///设置通信事件
		{
			if (GetCommState(m_hComm, &m_dcb))///获取当前DCB参数
			{
				...
				if (BuildCommDCB(szBaud, &m_dcb))///填写DCB结构//rxh1.0--网上新版本注释掉，但串口打开有警告，自己又加上的
				{
...
//
UINT CSerialPort::CommThread(LPVOID pParam)//rxh2.8 DWORD WINAPI改为了UINT
{
			...
			default:///发生其他错误，其中有串口读写中断开串口连接的错误
				{
					...
					::PostMessage((port->m_pOwner), WM_COMM_CLOSE_PORT, NULL, (LPARAM) port->m_nPortNr);//rxh2.8通知主线程，发生错误
...
{
	if (!(m_Thread = AfxBeginThread(CommThread, this,THREAD_PRIORITY_HIGHEST,0,0,NULL)))//rxh2.8更改线程优先级
...
void CSerialPort::ProcessErrorMessage(char* ErrorText)
{
	...
	sprintf(Temp, "         你是不是又把串口拔出来了！\n WARNING:  %s Failed with the following error: \n%s\nPort: %d \n", (char*)ErrorText, lpMsgBuf, m_nPortNr); 
	MessageBox(NULL, Temp, "串口错误", MB_ICONSTOP);
					...
					default:
						{
							...
							//port->ProcessErrorMessage("ReadFile()");//rxh2.5,取消弹出警告，防止出现两次警告弹窗
							return;///rxh2.3，提前到break前边可以执行，防止读写数据时，串口非正常断开导致死循环一直执行。add by itas109 2014-01-09 与上面liquanhai添加防死锁的代码差不多
							break;
						} 
				}
			}
	...
	if (cValues > 0) {
		for (i=0, retCode=ERROR_SUCCESS; i<cValues; i++) { 
			...
			if (ERROR_SUCCESS == RegEnumValue(hKey, i, achValue, &cchValue, NULL, NULL, NULL, NULL))  { 
				...
				if (-1 != szName.Find(_T("Serial")) || -1 != szName.Find(_T("VCom"))|| -1 != szName.Find(_T("Silabser")) || -1 != szName.Find(_T("NPtdrv")) || -1 != szName.Find(_T("VCP")) ){//rxh2.5增加|| -1 != szName.Find(_T("Silabser"))//使遍历时能够识别CP1202的串口转USB的芯片
...
void CSerialPort::Hkey2ComboBox(CComboBox& m_PortNO)
{
	...
	{
		CString szCom; 
		szCom.Format(_T("COM%d"), m_nComArray[i]);//格式存储
		m_PortNO.InsertString(i, szCom.GetBuffer(5));//插入字符串数据到i位置处
		++i;
		Flag = TRUE;
